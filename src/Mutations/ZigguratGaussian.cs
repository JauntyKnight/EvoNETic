// File copied from: https://github.com/colgreen/Redzen/blob/main/Redzen/Numerics/Distributions/Double/ZigguratGaussian.cs
...
public static class ZigguratGaussian
{
    ...
    public static void Sample(Random rng, out double x)
    {
        while(true)
        {
            ...
            // Convert to a ulong.
            ulong u = BitConverter.ToUInt64(buffer, 0);
    ...
    /// <param name="x">Reference to a variable to store the new sample value in.</param>
    public static void Sample(Random rng, double mean, double stdDev, out double x)
    {
        Sample(rng, out x);
    ...
    public static void Sample(Random rng, Span<double> span)
    {
    ...
    /// <param name="stdDev">Distribution standard deviation.</param>
    /// <param name="span">The span to fill with random samples.</param>
    public static void Sample(Random rng, double mean, double stdDev, Span<double> span)
    {
        for(int i=0; i < span.Length; i++)
        {
            Sample(rng, out span[i]);
    ...
    /// </summary>
    private static void SampleTail(Random rng, out double x)
    {
        double y;
        do
        {
